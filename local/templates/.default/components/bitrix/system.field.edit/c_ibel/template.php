...
if($arParams['arUserField']["SETTINGS"]["DISPLAY"] != "CHECKBOX")
{
	...
	{
		?>
		<select class="test" name="<?echo $arParams["arUserField"]["FIELD_NAME"]?>" size="<?echo $arParams["arUserField"]["SETTINGS"]["LIST_HEIGHT"]?>" <?=($arParams["arUserField"]["EDIT_IN_LIST"]!="Y"? ' disabled="disabled" ':'')?> >
		<?
		$bWasSelect = false;
		//ksort($arParams["arUserField"]["USER_TYPE"]["FIELDS"]);
		//asort($arParams["arUserField"]["USER_TYPE"]["FIELDS"]);
		foreach ($arParams["arUserField"]["USER_TYPE"]["FIELDS"] as $key => $val)
		{
			if($bWasSelect)
				$bSelected = false;
